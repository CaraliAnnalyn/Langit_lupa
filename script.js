...
// Hitbox definitions
const PLAYER_BODY_HITBOX = { xOffset: 6, yOffset: 8, width: 8, height: 12 };
const STONE_HITBOX = { xOffset: 2, yOffset: 2, width: 60, height: 30 };
  ...
  players.forEach((p, idx) => {
    ...
    platforms.forEach((platform) => {
      ...
      if (rectCollision(feetRect, stoneRect) && p.vy >= 0) {
        if (platform.safe) {
          p.y = stoneRect.y - 25 * SCALE + PLAYER_BODY_HITBOX.yOffset * SCALE;
          p.vy = 0;
          p.onGround = true;
          p.jumpsRemaining = p.maxJumps;
          if (wasInAir) playLandSound();
...
// --- Platform Countdown Bar ---
  ...
  // Update scoreboard
  const mode = GAME_MODES[currentGameMode];
  let html = `<b>📊 ${mode.name} Mode</b><br><small>Press Esc to end game</small><br><br>`;
